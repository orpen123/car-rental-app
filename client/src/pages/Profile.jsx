...
const Profile = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const map = {};
      reviewChecks.forEach(r => { map[r.id] = r; });
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      localStorage.setItem('user', JSON.stringify({ ...stored, name: res.data.name, phone: res.data.phone }));
  ...
  const stats = [
    ...
    { label: 'Active', value: bookings.filter(b => b.status === 'confirmed').length, color: 'text-green-600' },
    { label: 'Completed', value: bookings.filter(b => b.status === 'completed').length, color: 'text-purple-600' },
    { label: 'Cancelled', value: bookings.filter(b => b.status === 'cancelled').length, color: 'text-red-500' },
  ];

  const inputClass = 'w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all';
    ...
    <div className='min-h-screen bg-[#eaecf5] pt-20 pb-10'>
      <div className='max-w-6xl mx-auto px-4 sm:px-6 py-8'>
          ...
          <div className='flex flex-col sm:flex-row items-start sm:items-center gap-4'>
            ...
            <div className='flex-1'>
              <div className='flex items-center gap-2 mb-1'>
                ...
                {user?.role === 'admin' && (
                  <span className='text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-semibold'>Admin</span>
                )}
              </div>
            ...
            <div className='flex items-center gap-2 w-full sm:w-auto'>
              {user?.role === 'admin' && (
                <Link to='/admin' className='flex-1 sm:flex-none text-center px-4 py-2 bg-blue-50 text-blue-600 text-xs font-semibold rounded-xl hover:bg-blue-100 transition-colors'>
                  Dashboard
                </Link>
              )}
              <button onClick={handleLogout} className='flex-1 sm:flex-none px-4 py-2 bg-red-50 text-red-500 text-xs font-semibold rounded-xl hover:bg-red-100 transition-colors'>
                Logout
              </button>
            </div>
          </div>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5'>
          {stats.map((stat, i) => (
            <div key={i} className='bg-white rounded-2xl shadow-sm p-4 text-center hover:shadow-md transition-shadow'>
        ...
        <div className='bg-white rounded-2xl shadow-sm overflow-hidden'>
          <div className='flex border-b border-gray-100'>
            {['bookings', 'account'].map((tab) => (
              ...
              >
                {tab === 'bookings' ? `My Bookings (${bookings.length})` : 'Account Settings'}
              </button>
            ))}
          </div>
              ...
              <div>
                {loading ? (
                  ...
                  <div className='text-center py-16'>
                    ...
                    <p className='text-gray-400 font-medium text-sm mb-1'>No bookings yet</p>
                    <p className='text-gray-300 text-xs mb-5'>Start by browsing our fleet</p>
                    <Link to='/cars' className='inline-block px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl transition-colors'>
                      Browse Cars
                    </Link>
                  </div>
                ) : (
                          ...
                          <div className='flex flex-col sm:flex-row'>
                            ...
                            <div className='flex-1 p-4'>
                              <div className='flex items-start justify-between mb-2'>
                                <div>
                                  <h3 className='text-sm font-bold text-gray-900'>{booking.car?.brand} {booking.car?.model}</h3>
                              ...
                              <div className='flex flex-wrap gap-3 text-xs text-gray-400 mb-3'>
                                <span>🗓️ {booking.totalDays} day{booking.totalDays > 1 ? 's' : ''}</span>
                                {booking.pickupLocation && <span>📍 {booking.pickupLocation}</span>}
                              </div>

                              <div className='flex items-center justify-between pt-3 border-t border-gray-50'>
                                <span className='text-base font-black text-blue-600'>${booking.totalPrice}</span>
                ...
                <div className='bg-gray-50 rounded-2xl p-5 sm:p-6'>
                  <div className='flex items-center gap-3 mb-5'>
                    ...
                    <div>
                      ...
                      <p className='text-xs text-gray-400'>Update your name and phone</p>
                    </div>
                  </div>
                  <div className='space-y-4'>
                    ...
                    <div>
                      ...
                      <input type='tel' value={editForm.phone} onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })} placeholder='+212 6XX XXX XXX' className={`${inputClass} placeholder-gray-300`} />
                    </div>
                    <div>
                      <label className={labelClass}>Email Address</label>
                      <input type='email' value={user?.email || ''} readOnly className='w-full px-4 py-3 bg-gray-200 rounded-2xl text-sm text-gray-400 outline-none cursor-not-allowed' />
                    ...
                    {profileError && <div className='bg-red-50 border border-red-100 text-red-500 text-xs rounded-xl px-4 py-3'>{profileError}</div>}
                    <button onClick={handleUpdateProfile} disabled={profileLoading} className='w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white text-xs font-semibold rounded-2xl transition-all flex items-center justify-center gap-2'>
                      {profileLoading ? <><div className='w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin' />Saving...</> : <>
                        <svg className='w-3.5 h-3.5' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M5 13l4 4L19 7' /></svg>
                        Save Changes</>}
                    </button>
                  </div>
                ...
                <div className='bg-gray-50 rounded-2xl p-5 sm:p-6'>
                  ...
                  <div className='space-y-4'>
                    <div>
                      ...
                      <input type='password' value={passwordForm.currentPassword} onChange={(e) => setPasswordForm({ ...passwordForm, currentPassword: e.target.value })} placeholder='••••••••' className={`${inputClass} placeholder-gray-300`} />
                    </div>
                    <div>
                      <label className={labelClass}>New Password</label>
                      <input type='password' value={passwordForm.newPassword} onChange={(e) => setPasswordForm({ ...passwordForm, newPassword: e.target.value })} placeholder='••••••••' className={`${inputClass} placeholder-gray-300`} />
                    </div>
                    <div>
                      <label className={labelClass}>Confirm New Password</label>
                      <input type='password' value={passwordForm.confirmPassword} onChange={(e) => setPasswordForm({ ...passwordForm, confirmPassword: e.target.value })} placeholder='••••••••' className={`${inputClass} placeholder-gray-300`} />
                    </div>
                    ...
                    {passwordError && <div className='bg-red-50 border border-red-100 text-red-500 text-xs rounded-xl px-4 py-3'>{passwordError}</div>}
                    <button onClick={handleChangePassword} disabled={passwordLoading} className='w-full py-3 bg-gray-900 hover:bg-black disabled:bg-gray-300 text-white text-xs font-semibold rounded-2xl transition-all flex items-center justify-center gap-2'>
                      {passwordLoading ? <><div className='w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin' />Updating...</> : <>
      ...
      {reviewModal && (
        <div className='fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-4'>
          <div className='bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full' style={{ animation: 'scaleIn 0.2s ease-out forwards' }}>
            ...
            {/* Header */}
            <div className='flex items-center gap-3 mb-5'>
              <div className='w-10 h-10 bg-yellow-50 rounded-xl flex items-center justify-center text-xl flex-shrink-0'>⭐</div>
              <div>
                <h3 className='text-sm font-bold text-gray-900'>Leave a Review</h3>
                <p className='text-xs text-gray-400'>{reviewModal.car?.brand} {reviewModal.car?.model}</p>
              </div>
            </div>
            ...
            <div className='mb-4'>
              ...
              />
              <p className='text-xs text-gray-300 text-right mt-1'>{reviewForm.comment.length}/500</p>
            </div>
            ...
            <div className='flex gap-3'>
              ...
              <button
                onClick={() => { setReviewModal(null); setReviewError(''); setReviewForm({ rating: 5, comment: '' }); }}
      ...
      {cancelConfirm && (
        <div className='fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-4'>
          <div className='bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full' style={{ animation: 'scaleIn 0.2s ease-out forwards' }}>
            ...
            <h3 className='text-sm font-bold text-gray-900 text-center mb-1'>Cancel Booking</h3>
