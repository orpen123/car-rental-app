...
const ManageBookings = () => {
  ...
  const handleStatusUpdate = async (id, status, paymentStatus) => {
    ...
    try {
      await api.put(`/bookings/${id}`, { status, paymentStatus });
      setSuccess('Updated!');
      fetchBookings(); // Refresh data to update stats
      setTimeout(() => setSuccess(''), 2000);
    } catch (err) {
      setError('Update failed');
    } finally {
  ...
  // FIX: Calculate revenue only from confirmed and completed bookings that are paid
  // Exclude cancelled and refunded bookings from revenue
  const calculateRevenue = () => {
    return bookings
      .filter(booking => 
        // Only include bookings that are confirmed or completed
        (booking.status === 'confirmed' || booking.status === 'completed') &&
        // Only include paid bookings (not refunded or unpaid)
        booking.paymentStatus === 'paid'
      )
      .reduce((sum, booking) => sum + (booking.totalPrice || 0), 0);
  };

  // FIX: Calculate pending amount (unpaid but confirmed bookings)
  ...
  // Stats for bookings with corrected revenue
  const bookingStats = [
    { label: 'Total', value: bookings.length, icon: <FaClipboardList />, bg: 'bg-blue-50', text: 'text-blue-600' },
    { label: 'Pending', value: bookings.filter(b => b.status === 'pending').length, icon: <FaHourglassHalf />, bg: 'bg-amber-50', text: 'text-amber-600' },
    { label: 'Confirmed', value: bookings.filter(b => b.status === 'confirmed').length, icon: <FaCheckCircle />, bg: 'bg-emerald-50', text: 'text-emerald-600' },
