...
// // ✅ Lazy initialization — waits for dotenv to load
...
// export const sendBookingConfirmationEmail = async ({ name, email, booking, car }) => {
//   try {
//     await getResend.emails.send({
//       from: `${process.env.APP_NAME} <${process.env.FROM_EMAIL}>`,
//       to: email,
...
//               <!-- Total -->
//               <div style="background:linear-gradient(135deg,#eff6ff,#eef2ff);border-radius:16px;padding:20px;margin-bottom:24px;display:flex;justify-content:space-between;align-items:center;">
...
// export const sendBookingCancellationEmail = async ({ name, email, booking, car }) => {
//   try {
//     await getResend.emails.send({
//       from: `${process.env.APP_NAME} <${process.env.FROM_EMAIL}>`,
//       to: email,
